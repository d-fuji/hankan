...
export function AnnualSnapshotView({ year }: AnnualSnapshotViewProps) {
  ...
  return (
    <div className="space-y-6">
      {/* 将軍情報 */}
      {data.shogun && (
        <div className="rounded-lg border border-gold/30 bg-navy p-5 text-white">
          <p className="mb-1 text-sm text-white/60">征夷大将軍</p>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Link
                href={`/persons/${data.shogun.id}`}
                className="font-(family-name:--font-noto-serif) text-xl font-bold hover:text-gold"
              >
                {data.shogun.name}
              </Link>
      ...
      {/* 領地一覧 */}
      <div>
        <h2 className="mb-3 font-(family-name:--font-noto-serif) text-lg font-bold text-navy">
          領地一覧（{data.territories.length}件）
        </h2>
        {data.territories.length === 0 ? (
          <p className="py-4 text-center text-ink/50">該当する領地がありません</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-gold/20 text-left">
                  <th className="px-3 py-2 font-medium text-ink/60">領地名</th>
                  <th className="px-3 py-2 font-medium text-ink/60">種別</th>
                  <th className="px-3 py-2 font-medium text-ink/60">旧国</th>
                  <th className="px-3 py-2 font-medium text-ink/60">藩主/代官</th>
                  <th className="px-3 py-2 font-medium text-ink/60">家</th>
                  <th className="px-3 py-2 text-right font-medium text-ink/60">
                    石高（万石）
                  </th>
                </tr>
              </thead>
              <tbody>
                {data.territories.map((t) => (
                  <tr
                    key={t.id}
                    className="border-b border-gold/10 hover:bg-cream"
                  >
                    ...
                    <td className="px-3 py-2 text-ink/60">{t.territoryType}</td>
                    <td className="px-3 py-2 text-ink/60">{t.provinceName}</td>
                    <td className="px-3 py-2">
                      {t.lord ? (
                        <Link
